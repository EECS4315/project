...
public class Board {
    ...
    public List<Position> getPlayerPositions(int playerValue) {
        int size = this.boardValues.length;
        List<Position> playerValuePositions = new ArrayList<>();
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                if (boardValues[i][j] == playerValue)
                	playerValuePositions.add(new Position(i, j));
            }
        }
        return playerValuePositions;
    }
